from data_types import Activity, AdditionalParameters, ActivityExecution, Participant, Experiment, ParticipantState, \
    Participation


def get_activity_by_id(_id: str) -> Activity:
    # Search for activity with given ID in database
    return Activity(_id=1, name="example_activity",
...
def get_participant_by_id():
    return Participant(id=4, name="example_participant", additionalParameters=AdditionalParameters(
        key="example_key",
        value="example_value"))
